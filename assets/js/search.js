...
window.DOODLY_SEARCH = (function () {
  ...
  function trackSearch(q, count) { var c = cfg(), k = q.trim().toLowerCase(); if (k.length < 2) return; c.analytics.searches[k] = (c.analytics.searches[k] || 0) + 1; if (!count) c.analytics.noResult[k] = (c.analytics.noResult[k] || 0) + 1; saveCfg(c); logBackend(count ? "query" : "noresult", k, null, { resultCount: count || 0 }); try { if (window.DOODLY_ANALYTICS) DOODLY_ANALYTICS.trackSearch(k, count || 0); } catch (e) {} }
