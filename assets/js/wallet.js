...
window.DOODLY_WALLET = (function () {
  ...
  function adminRefund(id, amount, qty, note) { var w = find(id); if (!w) return; post(w, { kind: "credit", type: "refund", amount: Number(amount) || 0, desc: (note || "Bottle deposit refund") + (qty ? " · " + qty + " bottle(s)" : ""), by: me() }); saveWith(w); audit("wallet.refund", id); }
  ...
  function mountAdmin(host) {
    ...
    function wire() {
      if (state.tab === "wallets") {
        ...
        host.querySelectorAll(".w-rf").forEach(function (b) { b.addEventListener("click", function () { var a = prompt("Refund deposit amount (₹)"); if (a && Number(a) > 0) { var q = parseInt(prompt("Bottles returned (qty, optional)") || "", 10); adminRefund(b.dataset.id, Number(a), q > 0 ? q : 0, prompt("Note? (optional)") || ""); toast("Deposit refunded"); render(); } }); });
