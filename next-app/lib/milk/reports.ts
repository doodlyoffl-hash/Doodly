...
export async function buildMilkReport(type: MilkReportType, fromIso: string, toIso: string): Promise<MilkReport> {
  ...
  const rows: string[][] = [
    ...
    ["Retail — delivered", `${p.retailDeliveries} delivery/ies · ${p.retailCustomersServed} customer(s)`],
    ["Retail — milk delivered", `${n2(p.retailLitresDelivered)} L`],
    ["Avg selling price (retail)", retailAspPaise > 0 ? `${rup(retailAspPaise)}/L` : "—"],
    ["B2B — delivered", `${p.b2bDeliveries} order(s) · ${p.b2bBusinessesServed} business(es)`],
    ["B2B — milk delivered", `${n2(p.b2bLitresDelivered)} L`],
    ["Total milk delivered", `${n2(p.totalMilkLitresDelivered)} L`],
