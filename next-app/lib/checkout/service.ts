...
export async function placeOrder(userId: string, input: CheckoutInput, ctx: ReqContext) {
  ...
  /* ---- 1b. coupon (server-validated) + wallet (server-capped) — NEVER trust the
  ...
  if (payablePaise <= 0) {
    ...
    await audit({ userId, actorRole: "customer", action: "order.placed", target: `${base.number} wallet ₹${walletAppliedPaise / 100}`, ctx });
    await notifyOrderConfirmed(userId, { number: base.number });
