...
import { updateDeliverableAddress } from "@/lib/addresses/create";
...
export const PATCH = route("addresses.update", async (req: NextRequest, { params }: Ctx) => {
  ...
  // ONE shared deliverable-address edit path (re-verify + recompute on any location change).
  const address = await updateDeliverableAddress(userId, params.id, parsed as Record<string, unknown>, { actorRole: "customer", actorUserId: userId, ctx: reqContext(req) });
  return ok({ address, needsPin: false, verified: address.verified });
});
