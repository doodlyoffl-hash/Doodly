...
import { listInvoices, createInvoiceForOrder, b2bInvoiceSummary, type InvoiceSort, type InvoiceFilterArgs } from "@/lib/b2b/invoices";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const filters: InvoiceFilterArgs = {
    ...
    const [result, summary] = await Promise.all([
      listInvoices({ ...filters, sort: (sp.get("sort") as InvoiceSort) ?? undefined, limit: sp.get("limit") ? Number(sp.get("limit")) : undefined, offset: sp.get("offset") ? Number(sp.get("offset")) : undefined }),
      sp.get("summary") === "1" ? b2bInvoiceSummary(filters) : Promise.resolve(null),
    ]);
